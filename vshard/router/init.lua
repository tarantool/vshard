...
local function buckets_group(router, bucket_ids, timeout)
    ...
    return replicaset_buckets, timeout
end
...
--
local function router_map_callrw_prepare(router, timeout, mode, bucket_ids)
    local err, err_id
    local replicasets_to_wait, grouped_buckets = {}, {}
    ...
    return timeout, grouped_buckets
end
...
local function router_ref_storage_all(router, bucket_ids, timeout, rid)
    ...
    for rs_id in pairs(replicasets) do
        ...
        local target
        if buckets then
            args_ref = {'storage_ref_make_with_buckets',
                        rid, timeout, buckets, {force = true}}
            target = future_buckets
        else
            args_ref = {'storage_ref', rid, timeout}
            target = future_all
        end
        local res, ref_err = replicasets[rs_id]:callrw(
    ...
    timeout, err, err_id = router_map_callrw_ref_wait(
        futures, timeout)
    if not timeout then
        goto fail
    end
    ...
    for _, res in pairs(results) do
        if not res.total then
            ...
            err = lerror.vshard(lerror.code.UNSUPPORTED,
...
local function router_map_callrw(router, func, args, opts)
    ...
    if timeout then
        ...
        opts = {timeout = timeout, return_raw = do_return_raw}
