...
BucketRef(state, bucket, mode) ==
    ...
    IF mode = "read" THEN
        ...
        ELSE [state EXCEPT !.bucketRefs[bucket].ro = @ + 1]
    ELSE
...
\* It checks SENT buckets that are not transfering and have no refs.
...
\* Swap adjacent elements at positions k and k+1 (1-based).
\* If k not in 1..Len(S)-1, return S unchanged.
SeqSwapAdjacent(S, k) ==
  IF k < 1 \/ k >= Len(S) THEN S
  ELSE
    LET pre   == IF k = 1 THEN << >> ELSE SubSeq(S, 1, k-1)
        mid1  == S[k]
        mid2  == S[k+1]
        post  == IF k+1 = Len(S) THEN << >> ELSE SubSeq(S, k+2, Len(S))
    IN pre \o << mid2, mid1 >> \o post
