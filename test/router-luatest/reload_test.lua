...
local function prepare_temp_vshard_workspace(g)
    ...
    g.vshard_copy_path_load = '/vshard_copy'
...
local function remove_temp_vshard_workspace(g)
...
g.before_all(function()
    prepare_temp_vshard_workspace(g)
    create_cluster_on_specific_version(g, g.latest_hash)
end)

g.after_all(function()
    remove_temp_vshard_workspace(g)
    g.cluster:drop()
end)
...
g2.before_all(function()
    prepare_temp_vshard_workspace(g2)
...
g2.after_all(function()
    remove_temp_vshard_workspace(g2)
    g2.cluster:drop()
end)
